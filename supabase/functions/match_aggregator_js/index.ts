...
Deno.serve(async (req) => {
  ...
  if (!data || data.length === 0) {
    return new Response(`No Data Found for ${params.get('season')}${params.get('event')}`,
      { status: 404 });
  }
  if (params.get('type') === "text") {
    // Data Aggregation
    const agg: {[key: number]: {[key: string]: {[key: string]: string}}} = {}; // {team: {match: {question: value}}}
    for (const scoutingEntry of data) {
      const team: number = scoutingEntry["team"];
      ...
      ["event", "match", "team", "scouter"].forEach(k => delete scoutingEntry[k]);
    ...
    // Data Aggregation
    const agg: {[key: string]: {[key: number]: {[key: string]: Set<number>}}} = {}; // {match: {team: {scoretype: value}}}
    for (const scoutingEntry of data) {
      const match: string = scoutingEntry['match'];
    ...
    const matches: {[key: string]: {[key: number]: {[key: string]: number}}} =
    Object.fromEntries(Object.entries(agg).map(([k1, v1]: [string, {[key: number]: {[key: string]: Set<number>}}]) => [
      k1,
      Object.fromEntries(Object.entries(v1).map(([k2, v2]: [string, {[key: string]: Set<number>}]) => [
        k2,
        Object.fromEntries(Object.entries(v2).map(([k3, v3]: [string, Set<number>]) => [
          k3,
          isMedian
